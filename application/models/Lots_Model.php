    ...
    function farmer_marketing_search($farmerid, $state) {
        ...
        if ($state == "AP") {
            ...
            $query = "SELECT 
                       ...
                       (Farmer_Id='$farmerid' or Aadhar_No='$farmerid') and Record_Status='Active' and St_type!='AP'";
...
//        $lim = new LotInformationModel();
    ...
    function get_lots_list_based_on_farmer_id($Aadhar_No,$ACode){
            $query="SELECT distinct Lot_Id,RowId,SubLot_Id FROM `lot_information` where FarmerAadhar_Card='$Aadhar_No' and LotNet_Quantity is NULL AND DateOf_Marketing=DATE_FORMAT(CURRENT_DATE,'%y-%m-%d') AND Ac_Id='$ACode' AND Farmer_Agree='yes' GROUP BY Lot_Id";
            // log_message("error",'--'.$this->db->last_query());
            return $this->db->query($query)->result_array();
    ...
    function update_lots_details(){
        ...
        $update_array = array(
                            "Lot_Id" =>$lotid, "LotNet_Quantity" => $weight,
                            "NoOfCrates" => $noOfCrates, "WeightOfCrates" => $WeightOfCrates
                         );
